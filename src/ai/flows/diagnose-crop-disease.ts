...
const DiagnoseCropDiseaseInputSchema = z.object({
	...
	textDescription: z
		.string()
		.optional()
		.describe("Alternative description field for chat-based input."),
	outputFormat: z
		.enum(["detailed", "chat"])
		.optional()
		.default("detailed")
		.describe("Output format: detailed (structured) or chat (simple string)"),
});
...
const DiagnoseCropDiseaseChatOutputSchema = z.object({
	diagnosisResult: z
		.string()
		.describe(
			"The diagnosis result, including identified disease, confidence, and recommendations.",
		),
});

export type DiagnoseCropDiseaseDetailedOutput = z.infer<
	typeof DiagnoseCropDiseaseDetailedOutputSchema
>;
export type DiagnoseCropDiseaseChatOutput = z.infer<
	typeof DiagnoseCropDiseaseChatOutputSchema
>;
export type DiagnoseCropDiseaseOutput =
	| DiagnoseCropDiseaseDetailedOutput
	| DiagnoseCropDiseaseChatOutput;

// Follow-up question schemas
const DiagnoseFollowUpInputSchema = z.object({
	previousDiagnosis: DiagnoseCropDiseaseDetailedOutputSchema.describe(
		"The previous diagnosis result",
	),
	followUpQuestion: z
		.string()
		.describe(
			"The farmer's follow-up question about the diagnosis or management",
		),
	photoDataUri: z
		.string()
		.optional()
		.describe("Optional additional photo for follow-up analysis"),
});
...
const DiagnoseFollowUpOutputSchema = z.object({
	answer: z
		.string()
		.describe(
			"A detailed answer to the follow-up question, building on the previous diagnosis",
		),
	additionalRecommendations: z
		.string()
		.optional()
		.describe("Any additional recommendations based on the follow-up question"),
});
export type DiagnoseFollowUpOutput = z.infer<
	typeof DiagnoseFollowUpOutputSchema
>;
...
export async function diagnoseFollowUp(
	input: DiagnoseFollowUpInput,
): Promise<DiagnoseFollowUpOutput> {
...
const diagnoseCropDiseaseFlow = ai.defineFlow(
	{
		...
		outputSchema: z.union([
			DiagnoseCropDiseaseDetailedOutputSchema,
			DiagnoseCropDiseaseChatOutputSchema,
		]),
	},
	async (input) => {

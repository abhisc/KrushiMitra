...
export class SchemeService extends FirestoreService<GovernmentScheme> {
	...
	public buildQueryParams(params: SchemeSearchParams): any[] {
		...
		if (params.userProfile) {
			...
			if (profile.disability) {
				queryParams.push({ identifier: "disability", value: profile.disability });
			}
	...
	transformSchemeData(apiScheme: any, params: SchemeSearchParams): GovernmentScheme {
		return {
			schemeId: apiScheme.id || apiScheme.schemeId || `scheme_${Date.now()}`,
			name: apiScheme.name || apiScheme.title || 'Unknown Scheme',
			description: apiScheme.description || apiScheme.summary || 'No description available',
			eligibility: apiScheme.eligibility || 'Please check official website for eligibility criteria',
			benefits: apiScheme.benefits || apiScheme.amount || 'Benefits vary based on scheme',
			howToApply: apiScheme.howToApply || apiScheme.applicationProcess || 'Please visit official website for application process',
	...
	async getSchemesByCriteria(criteria: Partial<GovernmentScheme>): Promise<GovernmentScheme[]> {
		try {
			...
			return allSchemes.filter(scheme => {
				for (const [key, value] of Object.entries(criteria)) {
					if (value !== undefined && scheme[key as keyof GovernmentScheme] !== value) {
						return false;
					}
				}
				return true;

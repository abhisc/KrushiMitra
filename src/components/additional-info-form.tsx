...
import {
	UserService,
	UserProfile,
} from "@/firebaseStore/services/user-service";
	...
	const getCurrentLocation = async () => {
		...
		try {
			...
			toast({
				...
				description:
					"Coordinates obtained. Please enter your city and state manually.",
			});
		} catch (error) {
			toast({
				title: "Location Error",
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (formData.caste && formData.caste.trim()) {
				additionalInfo.caste = formData.caste as
					| "All"
					| "General"
					| "SC"
					| "ST"
			...
			if (formData.residence && formData.residence.trim()) {
				additionalInfo.residence = formData.residence as
					| "Both"
					| "Urban"
					| "Rural";
			}
			...
			if (formData.gender && formData.gender.trim()) {
				additionalInfo.gender = formData.gender as
					| "All"
					| "Male"
					| "Female"
					| "Other";
			...
			await userService.updateAdditionalInfo(
				user.uid,
				additionalInfo,
		...
		<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
			<Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
				<CardHeader>
					...
					<CardDescription>
						Help us provide personalized recommendations for weather, government
						schemes, and more. All fields are optional.
					</CardDescription>
				</CardHeader>
				<CardContent>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div className="space-y-2">
									...
									<Select
										...
										onValueChange={(value) =>
											handleInputChange("isStudent", value)
										}
									>
										<SelectTrigger>
								...
								<div className="space-y-2">
									...
									<Select
										...
										onValueChange={(value) =>
											handleInputChange("minority", value)
										}
									>
										<SelectTrigger>
											<SelectValue placeholder="Select option" />
								...
								<div className="space-y-2">
									...
									<Select
										...
										onValueChange={(value) =>
											handleInputChange("residence", value)
										}
									>
										<SelectTrigger>

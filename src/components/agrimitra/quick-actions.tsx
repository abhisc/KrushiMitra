...
function DiagnoseDialogContent({
	setOpen,
}: {
...
function MarketAnalysisDialogContent({
	setOpen,
}: {
	setOpen: (open: boolean) => void;
}) {
			...
			{result && (
				<div className="mt-4 space-y-4 rounded-lg border bg-secondary/50 p-4">
					<h3 className="font-bold">
						Market Analysis for {result.crop} in {result.market}
					</h3>
					<p>
...
function SchemeInfoDialogContent({
	setOpen,
}: {
	setOpen: (open: boolean) => void;
}) {
...
function WeatherTipsDialogContent({
	setOpen,
}: {
	setOpen: (open: boolean) => void;
}) {
	const { toast } = useToast();
	const [loading, setLoading] = useState(false);
	const [result, setResult] = useState<WeatherAndIrrigationTipsOutput | null>(
		null,
	);
		...
		<>
			...
			{result && (
				<div className="mt-4 space-y-4 rounded-lg border bg-secondary/50 p-4 max-h-[300px] overflow-y-auto">
					...
					<p>
						<strong>Forecast:</strong> {result.weatherForecast}
					</p>
					<p>
						<strong>Irrigation Tips:</strong> {result.irrigationTips}
					</p>
					<div className="mt-2">
...
const Dialogs: Record<
	string,
	React.FC<{ setOpen: (open: boolean) => void }>
> = {
...
export {
	MarketAnalysisDialogContent,
	SchemeInfoDialogContent,
	WeatherTipsDialogContent,
};

...
import { doc, setDoc, getDoc, updateDoc, collection, getDocs, query, where, orderBy, limit, deleteDoc } from 'firebase/firestore';
...
export class FirestoreService<T extends BaseDocument> {
  ...
  async createIfNotExists(id: string, data: Partial<T>): Promise<void> {
    try {
      ...
      console.error(`Error creating document if not exists in ${this.collectionName}:`, error);
      throw error;
    }
  }
  ...
  async getOrCreate(id: string, defaultData: Partial<T>): Promise<T> {
    try {
      ...
      console.error(`Error getting or creating document in ${this.collectionName}:`, error);
      throw error;
    }
  }
  ...
  async getOrdered(field: string, direction: 'asc' | 'desc' = 'asc', limitCount?: number): Promise<T[]> {
    try {
      let q = query(collection(db, this.collectionName), orderBy(field, direction));
      if (limitCount) {

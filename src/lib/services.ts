import { FirestoreService, BaseDocument } from './firestore-service';
...
export class GovernmentSchemeService extends FirestoreService<GovernmentScheme> {
  ...
  async getSchemesByEligibility(criteria: Partial<GovernmentScheme['eligibility']>): Promise<GovernmentScheme[]> {
    ...
    return activeSchemes.filter(scheme => {
      ...
      // Check each criteria
      if (criteria.isStudent !== undefined && eligibility.isStudent !== criteria.isStudent) {
        return false;
      }
      if (criteria.minority !== undefined && eligibility.minority !== criteria.minority) {
        return false;
      }
      if (criteria.disability !== undefined && eligibility.disability !== criteria.disability) {
        return false;
      }
      if (criteria.caste && eligibility.caste && !eligibility.caste.includes(criteria.caste[0])) {
        return false;
      }
      if (criteria.residence && eligibility.residence && !eligibility.residence.includes(criteria.residence[0])) {
        return false;
      }
      if (criteria.gender && eligibility.gender && !eligibility.gender.includes(criteria.gender[0])) {
        return false;
      }

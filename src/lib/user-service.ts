...
export class UserService extends FirestoreService<UserProfile> {
  ...
  async ensureUserProfile(userData: Partial<UserProfile>): Promise<UserProfile | null> {
    try {
  ...
  async updateAdditionalInfo(uid: string, additionalInfo: Partial<UserProfile>, userDisplayName?: string): Promise<void> {
    try {
      ...
      try {
        ...
        console.error('Error creating/updating user profile with upsert:', setError);
        throw setError;
      }
    }
  }
